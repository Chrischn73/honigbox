...
Diese Datei ist Teil des gemeinsamen Pi-Setup-Portals von HonigBox und der
Imker-App/BeeTown und existiert an DREI Orten - aber nur EINER davon ist
zum Bearbeiten gedacht:

    KANONISCH (hier bearbeiten):
        /media/SSD/Sichern/claude/pi-setup-portal/pi_setup_portal.py
        (eigener, bewusst App-unabhaengiger Ordner, kein Git-Repo - existiert
        nur, damit es fuer diesen Code genau EINE Bearbeitungsstelle gibt)

    NUR DEPLOYMENT-KOPIEN (NIEMALS direkt bearbeiten):
        /media/SSD/Sichern/claude/honigbox-webseite/setup/pi_setup_portal.py
        /media/SSD/Sichern/claude/imker-app/setup/pi_setup_portal.py

Falls diese Datei hier gerade UNTER honigbox-webseite/ oder imker-app/
liegt: nicht direkt bearbeiten, sondern die Aenderung am kanonischen
Ordner vornehmen und von dort aus weiterverteilen (siehe naechster Punkt) -
sonst geht die Aenderung beim naechsten Sync wieder verloren.

Fuer GitHub muss der Code trotzdem in BEIDEN App-Repos physisch vorhanden
sein (git submodule/subtree wuerden den GitHub-Tarball-basierten Update-
Mechanismus der Apps brechen, siehe die "honigbox-setup-portal"-Notiz in
Claudes Erinnerungen). Deshalb nach jeder Aenderung am kanonischen Ordner:

    1. PORTAL_VERSION weiter unten erhoehen.
    2. Im kanonischen Ordner "./sync.sh" ausfuehren - kopiert diese Datei
       (und pi-setup-portal.sh/.service, regen-issue.sh) unveraendert in
       beide App-Ordner oben.

Das gilt AUCH, wenn eine Aenderung nur "fuer HonigBox" oder nur "fuer die
Imker-App" angefordert wurde, aber tatsaechlich diese Datei betrifft (z.B.
Backup/Update/WLAN/Hilfe-Logik) statt eine der App-eigenen Dateien
(honigbox.sh, galerie_server.py, server.py usw.) - diese Datei hier ist
NIE app-spezifisch, sie liest alles App-spezifische aus apps.d/*.json.
=============================================================================
...
aus (PI_SETUP_LANDING_PORT), falls 80 beim Einrichten schon belegt war.
...
HOST = "0.0.0.0"
PORT_LANDING = int(os.environ.get("PI_SETUP_LANDING_PORT", "80"))
...
def load_apps():
    ...
    for name in names:
        ...
        try:
            ...
            print(f"pi_setup_portal: apps.d/{name} konnte nicht gelesen werden: {e}", file=sys.stderr)
            continue
        valid, reason = _validate_app_descriptor(app, name)
        if not valid:
            print(f"pi_setup_portal: apps.d/{name} ist unvollstaendig ({reason}) - ignoriert.", file=sys.stderr)
            continue
        apps.append(app)
...
if __name__ == "__main__":
    if "--check-update" in sys.argv:
        ...
        if not app_id:
            print("Nutzung: pi_setup_portal.py --check-update <app-id>", file=sys.stderr)
            sys.exit(1)
        app = get_app(app_id)
        if not app:
